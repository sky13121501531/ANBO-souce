...
bool TranslateDownXMLForRADIO::TranslateRecordSet(const std::string& strOriginalXML,std::string& strStandardXML)
{
	...
	{
		tempParser.SetAttrNode( "Priority",StrUtil::Int2Str(StrUtil::Str2Int(priority)+200),tempRootNode );
		...
		else		//录音任务设置
		{
			...
			for (int i=0;i!=nodelist->Size();++i)
			{
				...
				parser.GetAttrNode(childnode,"ExpireDays",ExpireDays);
				//
				StartDateTime = TimeUtil::DateTimeToStr(TimeUtil::StrToDateTime(StartDateTime)-5);
				EndDateTime = TimeUtil::DateTimeToStr(TimeUtil::StrToDateTime(EndDateTime)+5);
				StartTime = TimeUtil::TimeToStr(TimeUtil::StrToTime(StartTime)-5);
				EndTime = TimeUtil::TimeToStr(TimeUtil::StrToTime(EndTime)+5);
				//
...
bool TranslateDownXMLForRADIO::TranslateRecordTask(const std::string& strOriginalXML,std::vector<std::string>& strStandardXMLVec)
{
	...
	for(int i=0;i<recordSetList->Size();++i)
	{
		...
		//	DeviceID = string("-1");
		//}
		if(StrUtil::Str2Int(DeviceID)>0&&StrUtil::Str2Int(DeviceID)<17)
		{ // 录音通道为 9-16，通道必须合理
			DeviceID = StrUtil::Int2Str(StrUtil::Str2Int(DeviceID)+1);
		}
		else
		{//通道错误自动分配
			DeviceID = string("-1");
...
bool TranslateDownXMLForRADIO::TranslateRecordQuery(const std::string& strOriginalXML,std::string& strStandardXML)
{
	...
	size_t pos =0;
	pos = StartDateTime.find_first_of(":");
	if(pos != std::string::npos)
	{
		std::string curHour = StartDateTime.substr(pos-2,2);
		if("24" == curHour)
		{
			StartDateTime = StartDateTime.substr(0,pos-2)+ "00" + StartDateTime.substr(pos,StartDateTime.length());
		}
	}
	...
	if(Action=="Set"||Action=="set")
	{
		...
		{  
			...
			if (Code == "All" || Code == "all")
			{
				...
				CHANNELINFOMGR::instance()->GetChannelInfoByDvbtype(RADIO,vecChannelInfo);	//获取该类型频道表
				if (vecChannelInfo.empty())
				{
					string msg = "调频广播指标设置任务查询频道表失败!!";
		...
		for (int p = 0;p<parser.GetChildCount(qualityReportNode);p++)
		{
			...
			if (strNodeName =="CycleReportTime"||strNodeName =="SingleReportTime")
			{
				if (strNodeName =="CycleReportTime")
				{
					...
					parser.GetAttrNode(checkTimeNode,"StartDateTime",StartDateTime);          //获取原始XML中CycleReportTime节点的StartDateTime属性
					parser.GetAttrNode(checkTimeNode,"EndDateTime",EndDateTime);			  //获取原始XML中CycleReportTime节点的EndDateTime属性
				}
