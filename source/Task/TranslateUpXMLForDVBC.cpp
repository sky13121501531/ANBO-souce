...
//返回轮播查询XML
std::string TranslateUpXMLForDVBC::TranslateStreamRoundQuery(const XMLTask* task,std::string deviceID)
{
...
//返回自动录像节目文件下载XML
std::string TranslateUpXMLForDVBC::TranslateAutoRecordFile( const XMLTask* task,std::string URL,std::string startDateTime,std::string endDateTime)
{
...
std::string TranslateUpXMLForDVBC::TranslateRecordCapabilityQuery(const XMLTask* task)
{
	...
	{
		bool ret = PROPMANAGER::instance()->IsDeviceAvaiable(*pDeviceList,"AutoRecord",DVBC);
		if(ret)
			num++;
