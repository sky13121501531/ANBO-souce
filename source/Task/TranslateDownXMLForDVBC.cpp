...
//返回标准的视频轮播任务XML
bool TranslateDownXMLForDVBC::TranslateStreamRoundQuery(const std::string& strOriginalXML,std::string& strStandardXML)
{
	//视频查询标准的XML结构
	...
	for(int n=0;n<count;n++)
	{
		...
		for(int i=0;i<SetList->Size();i++)
		{
			...
			if(parser.GetNodeName(HoldNode)=="AlarmProgram")
			{
				...
				CHANNELINFOMGR::instance()->GetOrgNetID(DVBC,Freq,AudioPID,VideoPID,ServiceID,Netid);
				CHANNELINFOMGR::instance()->GetTsID(DVBC,Freq,AudioPID,VideoPID,ServiceID,Tsid);
				CHANNELINFOMGR::instance()->GetChannelID(DVBC,Netid,Tsid,ServiceID,VideoPID,AudioPID,"",Channelid);
