...
CASEntitlementQueryTask::CASEntitlementQueryTask() : DeviceIndependentTask()
{

}

CASEntitlementQueryTask::CASEntitlementQueryTask(std::string strXML) : DeviceIndependentTask(strXML)
{
}

CASEntitlementQueryTask::~CASEntitlementQueryTask()
{
}
void CASEntitlementQueryTask::Run(void)
{
	ACE_DEBUG ((LM_DEBUG,"(%T | %t)通道[%d]CAS 授权信息查询任务开始 !\n",DeviceID));
	...
	{
		SetFinised();
		ACE_DEBUG ((LM_DEBUG,"(%T | %t)通道[%d]CAS 授权信息查询任务获取时间失败 !\n",DeviceID));
		return;
	}
	...
	std::string recXML;
	std::string localurl;
	std::string reqURL="http://"+CasIp+":"+StrUtil::Int2Str(CasPort)+PATH+tmp+"_"+"CAS"+"_"+"EntitlementReport.zip";//构造请求的URL
	localurl="http://" + PROPMANAGER::instance()->GetHttpServerIP() + ":" + \
		PROPMANAGER::instance()->GetHttpServerPort() + HttpPath+tmp+"_"+"CAS"+"_"+"EntitlementReport.zip";
	...
	while(1)
	{
		...
		if(!isheader)
		{
			...
			{
				filename=LocalPath+tmp+"_"+"CAS"+"_"+"EntitlementReport.zip";
	...
	{
		bool xmlflag=false;
		reqURL="http://"+CasIp+":"+StrUtil::Int2Str(CasPort)+PATH+tmp+"_"+"CAS"+"_"+"EntitlementReport.xml";
		...
		if(xmlflag)
		{
			...
			filename=LocalPath+tmp+"_"+"CAS"+"_"+"EntitlementReport.xml";
			localurl="http://" + PROPMANAGER::instance()->GetHttpServerIP() + ":" + \
				PROPMANAGER::instance()->GetHttpServerPort() + HttpPath+tmp+"_"+"CAS"+"_"+"EntitlementReport.xml";
			XmlParser file;
			file.Set_xml(srcXML);
			...
			headParser.SetAttrNode( "Type",string("EntitlementQuery"),retNode );
			...
			size_t pos=srcXML.find("<EntitlementReport");
			if(pos!=string::npos)
			{
		...
		{
			char * cheader= "<?xml version=\"1.0\" encoding=\"GB2312\" standalone=\"yes\"?><Msg><Return></Return><EntitlementQueryReport></EntitlementQueryReport></Msg>";
			XmlParser headParser(cheader);
			pXMLNODE headRootNode = headParser.GetNodeFromPath( "Msg" );//根节点msg
			...
			headParser.SetAttrNode( "Redirect",string(""),retNode );//return节点的Comment属性
			pXMLNODE ReportNode=headParser.GetNodeFromPath("Msg/EntitlementQueryReport");
	...
	ACE_DEBUG ((LM_DEBUG,"(%T | %t)通道[%d]CAS 授权信息查询任务停止 !\n",DeviceID));
}

string CASEntitlementQueryTask::GetTaskName()
{
	return std::string("授权信息查询任务");
}
std::string CASEntitlementQueryTask::GetObjectName()
{
	return std::string("CASEntitlementQueryTask");
}
