...
{
	ACE_DEBUG ((LM_DEBUG,"(%T | %t)通道[%d]CAS 产品信息查询任务开始 !\n",DeviceID));
	...
	{
		SetFinised();
		ACE_DEBUG ((LM_DEBUG,"(%T | %t)通道[%d]CAS 产品信息查询任务获取时间失败 !\n",DeviceID));
		return;
	}
	...
	std::string reqURL="http://"+CasIp+":"+StrUtil::Int2Str(CasPort)+PATH+tmp+"_"+"CAS"+"_"+"ProductListReport.zip";//构造请求的URL
	...
	ACE_DEBUG ((LM_DEBUG,"(%T | %t)通道[%d]CAS 产品信息查询任务停止 !\n",DeviceID));
}

string CASProductListQueryTask::GetTaskName()
{
	return std::string("产品信息查询任务");
}
