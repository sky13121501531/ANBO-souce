...
//返回轮播查询XML
std::string TranslateUpXMLForTHREED::TranslateStreamRoundQuery(const XMLTask* task,std::string deviceID)
{
...
std::string TranslateUpXMLForTHREED::TranslateRecordCapabilityQuery(const XMLTask* task)
{
	...
	{
		bool ret = PROPMANAGER::instance()->IsDeviceAvaiable(*pDeviceList,"AutoRecord",THREED);
		if(ret)
			num++;
