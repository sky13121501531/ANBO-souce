...
CASStaticsQueryTask::CASStaticsQueryTask() : DeviceIndependentTask()
{

}

CASStaticsQueryTask::CASStaticsQueryTask(std::string strXML) : DeviceIndependentTask(strXML)
{
}
CASStaticsQueryTask::~CASStaticsQueryTask()
{
}
void CASStaticsQueryTask::Run(void)
{
	ACE_DEBUG ((LM_DEBUG,"(%T | %t)通道[%d]CAS 统计信息查询任务开始 !\n",DeviceID));
	...
	{
		SetFinised();
		ACE_DEBUG ((LM_DEBUG,"(%T | %t)通道[%d]CAS 统计信息查询任务获取时间失败 !\n",DeviceID));
		return;
	}
	...
	std::string reqURL="http://"+CasIp+":"+StrUtil::Int2Str(CasPort)+PATH+tmp+"_"+"CAS"+"_"+"StaticsReport.zip";//构造请求的URL
	std::string localurl="http://" + PROPMANAGER::instance()->GetHttpServerIP() + ":" + \
		PROPMANAGER::instance()->GetHttpServerPort() + HttpPath+tmp+"_"+"CAS"+"_"+"StaticsReport.zip";
	...
	while(1)
	{
		...
		if(!isheader)
		{
			...
			{
				filename=LocalPath+tmp+"_"+"CAS"+"_"+"StaticsReport.zip";
	...
	{
		bool xmlflag=false;
		reqURL="http://"+CasIp+":"+StrUtil::Int2Str(CasPort)+PATH+tmp+"_"+"CAS"+"_"+"StaticsReport.xml";
		...
		if(xmlflag)
		{
			...
			filename=LocalPath+tmp+"_"+"CAS"+"_"+"StaticsReport.xml";
			localurl="http://" + PROPMANAGER::instance()->GetHttpServerIP() + ":" + \
				PROPMANAGER::instance()->GetHttpServerPort() + HttpPath+tmp+"_"+"CAS"+"_"+"StaticsReport.xml";
			XmlParser file;
			file.Set_xml(srcXML);
			...
			headParser.SetAttrNode( "Type",string("StaticsQuery"),retNode );
			...
			size_t pos=srcXML.find("<StaticsReport");
			if(pos!=string::npos)
			{
		...
		{
			char * cheader= "<?xml version=\"1.0\" encoding=\"GB2312\" standalone=\"yes\"?><Msg><Return></Return><StaticsReport></StaticsReport></Msg>";
			XmlParser headParser(cheader);
			pXMLNODE headRootNode = headParser.GetNodeFromPath( "Msg" );//根节点msg
			...
			pXMLNODE ReportNode=headParser.GetNodeFromPath("Msg/StaticsReport");
	...
	ACE_DEBUG ((LM_DEBUG,"(%T | %t)通道[%d]CAS 统计信息查询任务停止 !\n",DeviceID));
}

string CASStaticsQueryTask::GetTaskName()
{
	return std::string("统计信息查询任务");
}
std::string CASStaticsQueryTask::GetObjectName()
{
	return std::string("CASStaticsQueryTask");
}
