...
SMSICCardQueryTask::SMSICCardQueryTask(void)
{
}

SMSICCardQueryTask::SMSICCardQueryTask( std::string strXML ):DeviceIndependentTask(strXML)
{
...
void SMSICCardQueryTask::Run()
{
	ACE_DEBUG ((LM_DEBUG,"(%T | %t)通道[%d]SMS IC卡信息查询任务开始执行 !\n",DeviceID));

	OSFunction::Sleep(5);	//多个sms任务同时执行的话，tomcat接收会出问题(会丢弃一些xml)

	std::string RetXml;//回复给上层页面的xml
	std::string Date;
	std::string InstQuery;
	XmlParser StdParser(strStandardXML.c_str());
	pXMLNODE TaskNode=StdParser.GetNodeFromPath("Msg/ICCardQuery");
...
//	OSFunction::Sleep(3);
	...
	if (RecvSMSXml.length()>0)		//接收到的xml不为空
	{
		...
		if (RedirectURL.length()>0)		//有重定向文件，即压缩包
		{
			...
			std::string ZipFileName=Date+"_SMS_ICCardQueryR.zip";
	...
	SetFinised();
	bRun = false;
	ACE_DEBUG ((LM_DEBUG,"(%T | %t)通道[%d]SMS IC卡信息查询任务停止 !\n",DeviceID));
	
}

std::string SMSICCardQueryTask::GetObjectName()
{
	return "SMSICCardQuery";
}
std::string SMSICCardQueryTask::GetTaskName()
{
	return "SMS IC卡信息查询";
}

bool SMSICCardQueryTask::CreateRXml( std::string URL,std::string XmlFromSMS,std::string& RXml )
{
