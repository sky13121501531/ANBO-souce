...
SMSCAProductQuery::SMSCAProductQuery(void)
{
}
...
SMSCAProductQuery::SMSCAProductQuery(std::string strXML):DeviceIndependentTask(strXML)
{
	strStandardXML=strXML;
}

void SMSCAProductQuery::Run( void )
{
	ACE_DEBUG ((LM_DEBUG,"(%T | %t)通道[%d]SMS CA产品信息查询任务开始执行 !\n",DeviceID));

	OSFunction::Sleep(15);		//多个sms任务同时执行的话，tomcat接收会出问题(会丢弃一些xml)

	std::string RetXml;//回复给上层页面的xml
	std::string Date;
	std::string InstQuery;
	XmlParser StdParser(strStandardXML.c_str());
	pXMLNODE TaskNode=StdParser.GetNodeFromPath("Msg/CAProductQuery");
...
//	OSFunction::Sleep(3);
	...
	if (RecvSMSXml.length()>0)		//接收到的xml不为空
	{
		...
		if (RedirectURL.length()>0)		//有重定向文件，即压缩包
		{
			...
			std::string ZipFileName=Date+"_SMS_CAProductQueryR.zip";
	...
	SetFinised();
	bRun = false;
	ACE_DEBUG ((LM_DEBUG,"(%T | %t)通道[%d]SMS CA产品信息查询任务停止 !\n",DeviceID));
}

bool SMSCAProductQuery::CreateRXml( std::string URL,std::string XmlFromSMS, std::string& RXml )
{
...
std::string SMSCAProductQuery::GetObjectName()
{
	return "SMSCAProductQuery";
}

std::string SMSCAProductQuery::GetTaskName()
{
	return "SMS CA产品查询任务";
}