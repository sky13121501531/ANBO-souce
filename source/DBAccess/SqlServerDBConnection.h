...
public:
	...
	bool QueryISTask(eDVBType type,std::string xml);//某类型的任务是否存在，存在返回true ,否则返回false 查询任务
	...
	bool DeleteSpecDataExpire();//删除过期频谱数据 默认30天
	bool DeleteQualityDataExpire();//删除过期指标数据 默认30天
